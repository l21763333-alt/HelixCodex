...
def sync_remote_base_via_subagent() -> dict[str, Any]:
    cfg = get_config().mcp.git
    prompt = f"""
...
Required steps:
1. Call sync_remote_base(remote={cfg.remote!r}, base_branch={cfg.base_branch!r}).
2. Call get_model_repo_state.
3. Return JSON only:
{{
  "ok": true|false,
...
- push: {bool(cfg.push_on_keep)}
- create_pr: {bool(cfg.create_pr_on_keep)}

Required steps:
1. Call publish_keep_result with the inputs above.
2. Call get_model_repo_state.
3. Return JSON only:
{{
  "ok": true|false,
...
- trial_id: {trial_id!r}
- branch: {branch!r}
- target_branch: {(cfg.push_target_branch or branch)!r}
- base_branch: {cfg.base_branch!r}
- report_path: {str(report_path)!r}
...
Required steps:
1. If push is true and commit.committed is true, call push_model_trial_branch(branch=branch, remote={cfg.remote!r}, target_branch={(cfg.push_target_branch or branch)!r}).
2. If create_pr is true and commit.committed is true, call create_model_pr(branch=branch, base={cfg.base_branch!r}, draft={bool(cfg.pr_draft)}, title="forecast: keep {trial_id}", body=<metrics summary>).
3. Call get_model_repo_state.
4. Return JSON only:
{{
  "ok": true|false,
